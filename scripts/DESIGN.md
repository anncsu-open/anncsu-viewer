...
During ingestion, each address is validated against the official ISTAT municipality boundary polygons using a spatial containment check:
...
3. Check `ST_Contains(boundary, point)` — if the point falls outside → flag as `out_of_bounds`
